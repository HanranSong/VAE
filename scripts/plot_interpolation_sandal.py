...
def main():
    ...
    for img, label in dataset:
        if label == 5:
            corners.append(img.unsqueeze(0).to(device))
        if len(corners) == 4:
            break

    with torch.no_grad():
        mu_tl, _ = model.encode(corners[0])  # Top-Left
        mu_tr, _ = model.encode(corners[1])  # Top-Right
        mu_bl, _ = model.encode(corners[2])  # Bottom-Left
        mu_br, _ = model.encode(corners[3])  # Bottom-Right
        ...
        # Save the grid
        save_path = os.path.join(run_dir, "images", "latent_interpolation_sandal.png")
